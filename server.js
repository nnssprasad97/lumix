...
//   process.env.GITHUB_CLIENT_SECRET || "NOT FOUND"
// );
...
app.use("/github/dashboard.html", isAuthenticated);
app.use("/github/repo.html", isAuthenticated);
...
apiRouter.post("/ai-search", async (req, res) => {
  ...
  try {
    ...
    const repoNameMatch = query.match(
      /in\s+(?:repo|repository)\s+["']?([^"'\s]+)["']?/i
    );
    ...
    const repoTreePromises = reposToSearch.map(async (repo) => {
      try {
        ...
        return {
          ...
          filePaths: treeResponse.data.tree
    ...
    if (fileTypeKey) {
      fileTypeInstruction = `\nUser is searching for all files of type: ${fileTypeKey.toUpperCase()} (${fileTypeMap[
        fileTypeKey
      ].join(
        ", "
...
Prioritize files that directly match the query terms. If searching for a file type, include ALL files of that type across repositories.`;
    ...
    if (aiResponse.results && Array.isArray(aiResponse.results)) {
      for (const result of aiResponse.results) {
        const repoTree = validRepoTrees.find(
          (rt) => rt.repo.name === result.repoName
        );
